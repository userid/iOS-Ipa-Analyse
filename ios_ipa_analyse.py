...
def write_link_map_to_result_file(size_map, link_mpa_file_path, link_map_result_file_path, analyse_group):
    ...
    for key in size_map:
        ...
        if "size" in symbol:
            ...
            print("WARN : some error occurred for key", end=' ')
            print(key)
    # 根据map对文件进行排序
    a_file_sorted_list = sorted(list(a_file_map.items()), key=lambda x: x[1], reverse=True)
    print("%s" % "=".ljust(80, '='))
...
def red_link_map_file(base_link_map_file_path, link_map_result_file_path):
    try:
        link_map_file = open(base_link_map_file_path, encoding='latin1')
    except IOError:
        ...
        try:
            ...
            if not check_link_map_content(content):
                ...
                pass
            link_map_file_tmp = open(base_link_map_file_path,encoding='latin1')
